...
def plot_power_flow(m, G, i, j, filename="Figures/Powerflow.pdf"):
    ...
    for n in G.nodes():
        ...
        if n in m.children:
            plt.text(
                x, y - 0.1, text, fontsize=10, ha="center", va="top", color="red"
            )  # Color children interval red
        elif n in m.parents:
            plt.text(
                x, y + 0.1, text, fontsize=10, ha="center", va="bottom", color="black"
            )  # Color parent bounds black
        else:
            plt.text(
                x, y, text, fontsize=8, ha="center", va="center", color="black"
...
def plot_alloc_alpha(
    ...
    if show:
        ...
        plt.plot(
            alpha_values, envelope, marker="o", linestyle="-", label="Envelope Volume"
        )
