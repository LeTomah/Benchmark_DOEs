...
def constraints(m, G):
    ...
    def current_bounds_rule(m, u, v, vert_pow, vert_volt):
        ...
        return pyo.inequality(
            m.I_min[u, v], m.I[u, v, vert_pow, vert_volt], m.I_max[u, v]
        )
    ...
    def current_def_rule(m, u, v, vert_pow, vert_volt):
        ...
        return (
            m.I[u, v, vert_pow, vert_volt] * m.V_P[vert_volt]
            == m.F[u, v, vert_pow, vert_volt]
        )
    ...
    def power_balance_rule(m, n, vert_pow, vert_volt):
        ...
        if n in m.parents:
            return (
                expr == m.E[n, vert_pow, vert_volt] - m.P_plus[n, vert_pow, vert_volt]
            )
        if n in m.children:
            return (
                expr == m.E[n, vert_pow, vert_volt] + m.P_minus[n, vert_pow, vert_volt]
            )
        else:
    ...
    m.parent_power_constraint = pyo.Constraint(
        m.parents, m.i, m.j, rule=parent_power_constraint_rule
    )
    ...
    m.children_voltage = pyo.Constraint(
        m.children, m.i, m.j, rule=children_voltage_rule
    )
    ...
    def diff_bis_dso_rule(m, u):
        return (m.P_C_set[u, 0] + m.P_C_set[u, 1]) / 2 - m.info_DSO_param[
            u
        ] <= m.diff_DSO[u]
    ...
    m.net_power_upper = pyo.Constraint(
        m.PositiveNodes, m.i, m.j, rule=net_power_upper_rule
    )
    ...
    m.net_power_lower = pyo.Constraint(
        m.NegativeNodes, m.i, m.j, rule=net_power_lower_rule
    )
