...
def build(model: pyo.ConcreteModel, graph: Any) -> None:
    ...
    model.R = pyo.Param(
        model.Lines,
        initialize={(u, v): float(graph[u][v].get("R", 0.0)) for (u, v) in lines},
        mutable=True,
    )
    model.X = pyo.Param(
        model.Lines,
        initialize={(u, v): float(graph[u][v].get("X", 0.0)) for (u, v) in lines},
        mutable=True,
    )
    model.Z2 = pyo.Param(
        model.Lines,
        initialize={
            (u, v): float(graph[u][v].get("R", 0.0)) ** 2
            + float(graph[u][v].get("X", 0.0)) ** 2
            for (u, v) in lines
        },
        mutable=True,
    )
    ...
    model.G = pyo.Var(model.Lines, domain=pyo.Reals)
    model.I_sqr = pyo.Var(model.Lines, domain=pyo.NonNegativeReals)
    model.V_sqr = pyo.Var(model.Nodes, domain=pyo.NonNegativeReals)

    # Nodal injections and exchanges with outside of the operational graph
    model.E = pyo.Var(model.Nodes, domain=pyo.Reals)
    model.H = pyo.Var(model.Nodes, domain=pyo.Reals)
    model.P_plus = pyo.Var(model.Nodes, domain=pyo.Reals)
    model.P_minus = pyo.Var(model.Nodes, domain=pyo.Reals)
    model.Q_plus = pyo.Var(model.Nodes, domain=pyo.Reals)
    model.Q_minus = pyo.Var(model.Nodes, domain=pyo.Reals)
    ...
    model.active_balance = pyo.Constraint(model.Nodes, rule=active_balance_rule)
    ...
    model.reactive_balance = pyo.Constraint(model.Nodes, rule=reactive_balance_rule)
    ...
    model.voltage_drop = pyo.Constraint(model.Lines, rule=voltage_drop_rule)

    def current_voltage_rule(m, n, j):
        return m.V_sqr[n] * m.I_sqr[n, j] >= m.F[n, j] ** 2 + m.G[n, j] ** 2

    model.current_voltage = pyo.Constraint(model.Lines, rule=current_voltage_rule)
